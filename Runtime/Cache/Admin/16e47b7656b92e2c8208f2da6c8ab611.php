...
<div class="panel-header-container <?php echo ($crumbs_hide); ?>">
            ...
            <tr>
                <th width="100px">
                    <select id="search_key" name="search_key" style=" width: 100px">
                        <option selected="selected" value="FullName">返点名称</option>
                    </select>
                </th>
                <td width="300px">
    ...
    <table id="tg_main"
        ...
        <thead>
        <tr>
            <th field="RateID" checkbox="true"></th>
            <th field="RateName" sortable="true" >返点名称</th>
            <th field="Rate" sortable="true" >返点类型</th>
            <th field="Amount" sortable="true">返点金额</th>
            <th field="Commission" sortable="true" >佣金类型</th>
    ...
    <script type="text/javascript">
        ...
        function btnEdit_Click(row_id) {
            //获取选中行的id
            if (!row_id) {
                row_id = get_checked_row("#tg_main", "RateID");
            }
            if (row_id) {
                top.showModal(
                        '/admin/Agent/editAgentRate?id=' + row_id,
                        '编辑', //调用语言来设置标题
                        {
                            width: 870,
                            height: 500, //窗口高度
        ...
        function btnAdd_Click() {
            top.showModal(
                    '/admin/Agent/addAgentRate'  , //使用C#生成url
                    '新增' , //调用语言来设置标题
                    {
                        width: 870,
                        height: 500, //窗口高度
        ...
        function btnRemove_Click() {
            //获取当前勾选的所有行id
            var row_ids = get_checked_rows("#tg_main", "RateID");
            if (row_ids && confirm_delete()) {
                $.post(
                        '/admin/Agent/delAgentRate' ,
                        {
                            ids : row_ids
                        },
                        function(data) {
