...
<div class="panel-header-container <?php echo ($crumbs_hide); ?>">
            ...
            <tr>
                <th width="100px">
                    <select id="search_key" name="search_key" style=" width: 100px">
                        <option selected="selected" value="UserName">部门名称</option>
    ...
    <table id="tg_main"
        ...
        <thead>
        <tr>
            <th field="DepartmentID" checkbox="true"></th>
            <th field="DepartmentName" sortable="true"  align="center">部门名称</th>
            <th field="DepartmentNum" sortable="true"  align="center">部门编号</th>
    ...
    <script type="text/javascript">
        ...
        $("#tg_main").datagrid({
            onDblClickRow: function (index, row) {
                btnEdit_Click(row.DepartmentID);
            }
        });
        ...
        function btnEdit_Click(row_id) {
            //获取选中行的id
            if (!row_id) {
                row_id = get_checked_row("#tg_main", "DepartmentID");
            }
            if (row_id) {
                top.showModal(
                        '/admin/Employee/editDepartment?id=' + row_id,
                        '编辑', //调用语言来设置标题
                        {
                            width: 570,
                            height: 230, //窗口高度
        ...
        function btnAdd_Click() {
            top.showModal(
                    '/admin/Employee/addDepartment'  , //使用C#生成url
                    '新增' , //调用语言来设置标题
                    {
                        width: 570,
                        height: 230, //窗口高度
        ...
        function btnRemove_Click() {
            //获取当前勾选的所有行id
            var row_ids = get_checked_rows("#tg_main", "DepartmentID");
            if (row_ids && confirm_delete()) {
                $.post(
                        '/admin/Employee/delDepartment',
                        {
                            ids : row_ids
                        },
                        function(data) {
