...
<div class="panel-header-container <?php echo ($crumbs_hide); ?>">
    ...
    <form class="form-main" id="form-main" action="">
        <input type="hidden" name="id" value="<?php echo ($mInfo["MarketID"]); ?>" />
        <table class="form-table" border="0">
            <tr>
                <th>客户</th>
                <td>
                    <select name="Identifier" class="form-control">
                        <?php if(is_array($cInfo)): $i = 0; $__LIST__ = $cInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><option value="<?php echo ($c["Identifier"]); ?>" <?php if($c['Identifier'] == $mInfo['Identifier']): ?>selected<?php endif; ?>>
                                <?php echo ($c["FullName"]); ?>
