<?php if (!defined('THINK_PATH')) exit();?><div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel"><?php if(empty($_GET['id'])): ?>新增<?php else: ?> 编辑<?php endif; ?>问题优先级</h4>
</div>
<div class="modal-body">
    <form class="form-horizontal" role="form" id="defaultForm" method="post" action="<?php echo ($self); ?>">
        <input type="hidden" name="id" value="<?php echo ($info["FbPriorityID"]); ?>" />
        <fieldset>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="FbPriorityName">优先级名称</label>
                <div class="col-sm-4">
                    <input class="form-control" id="FbPriorityName" name="FbPriorityName" type="text" value="<?php echo ($info["FbPriorityName"]); ?>"/>
                </div>
            </div>
