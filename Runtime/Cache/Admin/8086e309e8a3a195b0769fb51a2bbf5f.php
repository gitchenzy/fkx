...
<div class="panel-header-container <?php echo ($crumbs_hide); ?>">
            ...
            <tr>
                <th width="100px">
                    <select id="search_key" name="search_key" style=" width: 100px">
                        <option selected="selected" value="CttNumber">合同编号</option>
    ...
    <table id="tg_main"
        ...
        <thead>
        <tr>
            <th field="ContractID" checkbox="true"></th>
            <th field="CustomerName" sortable="true"  align="center">客户</th>
            <th field="CttTypeName" sortable="true" >合同类型</th>
            <th field="CttNumber" sortable="true" >合同编号</th>
            <th field="ProductName" sortable="true" >购买产品</th>
            <th field="Identifier" sortable="true" >客户编号</th>
            <th field="Amount" >总金额</th>
            <th field="Charge" >维护费用</th>
            <th field="CtmSign" >客户签约人</th>
            <th field="OwnSign" >我方签约人</th>
            <th field="SignTime">签约时间</th>
            <th field="BeginTime" >生效时间</th>
            <th field="FinishTime" >结束时间</th>
        </tr>
        </thead>
    </table>
    ...
    <script type="text/javascript">
        ...
        function btnEdit_Click(row_id) {
            //获取选中行的id
            if (!row_id) {
                row_id = get_checked_row("#tg_main", "ContractID");
            }
            if (row_id) {
                top.showModal(
                        '/admin/Market/editContract?id=' + row_id,
                        '编辑', //调用语言来设置标题
                        {
                            width: 870,
                            height: 590, //窗口高度
        ...
        function btnAdd_Click() {
            top.showModal(
                    '/admin/Market/addContract'  , //使用C#生成url
                    '新增' , //调用语言来设置标题
                    {
                        width: 870,
                        height: 590, //窗口高度
        ...
        function btnRemove_Click() {
            //获取当前勾选的所有行id
            var row_ids = get_checked_rows("#tg_main", "ContractID");
            if (row_ids && confirm_delete()) {
                $.post(
                        '/admin/Market/delContract' ,
                        {
                            ids : row_ids
                        },
                        function(data) {
