...
<div class="panel-header-container <?php echo ($crumbs_hide); ?>">
            ...
            <tr>
                <th width="100px">
                    <select id="search_key" name="search_key" style=" width: 100px">
                        <option selected="selected" value="UserName">支付名称</option>
    ...
    <table id="tg_main"
        ...
        <thead>
        <tr>
            <th field="PayID" checkbox="true"></th>
            <th field="PayName" sortable="true"  align="center">支付名称</th>
            <th field="TypeName" sortable="true"  align="center">支付类型</th>
            <th field="AccountNum" sortable="true"  align="center">收款账号</th>
    ...
    <script type="text/javascript">
        ...
        function btnEdit_Click(row_id) {
            //获取选中行的id
            if (!row_id) {
                row_id = get_checked_row("#tg_main", "PayID");
            }
            if (row_id) {
                top.showModal(
                        '/admin/Customer/editPayType?id=' + row_id,
                        '编辑', //调用语言来设置标题
                        {
                            width: 570,
                            height: 300, //窗口高度
        ...
        function btnAdd_Click() {
            top.showModal(
                    '/admin/Customer/addPayType'  , //使用C#生成url
                    '新增' , //调用语言来设置标题
                    {
                        width: 570,
                        height: 300, //窗口高度
        ...
        function btnRemove_Click() {
            //获取当前勾选的所有行id
            var row_ids = get_checked_rows("#tg_main", "PayID");
            if (row_ids && confirm_delete()) {
                $.post(
                        '/admin/Customer/delPayType',
                        {
                            ids : row_ids
                        },
                        function(data) {
