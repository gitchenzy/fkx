...
<div class="panel-header-container <?php echo ($crumbs_hide); ?>">
            ...
            <tr>
                <th width="100px">
                    <select id="search_key" name="search_key" style=" width: 100px">
                        <option selected="selected" value="CompanyName">公司名称</option>
    ...
    <table id="tg_main"
        ...
        <thead>
        <tr>
            <th field="AgentID" checkbox="true"></th>
            <th field="Sort" sortable="true"  align="center">排序</th>
            <th field="LoginName" sortable="true" >账号</th>
            <th field="CompanyName" sortable="true" >公司名称</th>
            <th field="Identifier" sortable="true" >代理编号</th>
            <th field="Principal" sortable="true" >负责人</th>
            <th field="Telephone" sortable="true" >联系电话</th>
            <th field="Mobile"  >手机</th>
            <th field="Wechat" >微信</th>
            <th field="Email" >邮箱</th>
            <th field="AgentTime" >加入时间</th>
        </tr>
        </thead>
    </table>
    ...
    <script type="text/javascript">
        ...
        function btnEdit_Click(row_id) {
            //获取选中行的id
            if (!row_id) {
                row_id = get_checked_row("#tg_main", "AgentID");
            }
            if (row_id) {
                top.showModal(
                        '/admin/Agent/editAgent?id=' + row_id,
                        '编辑', //调用语言来设置标题
                        {
                            width: 870,
                            height: 590, //窗口高度
        ...
        function btnAdd_Click() {
            top.showModal(
                    '/admin/Agent/addAgent'  , //使用C#生成url
                    '新增' , //调用语言来设置标题
                    {
                        width: 870,
                        height: 590, //窗口高度
        ...
        function btnRemove_Click() {
            //获取当前勾选的所有行id
            var row_ids = get_checked_rows("#tg_main", "AgentID");
            if (row_ids && confirm_delete()) {
                $.post(
                        '/admin/Agent/delAgent' ,
                        {
                            ids : row_ids
                        },
                        function(data) {
