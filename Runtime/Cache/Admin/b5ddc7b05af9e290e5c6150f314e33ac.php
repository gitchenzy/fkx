...
<div class="panel-header-container <?php echo ($crumbs_hide); ?>">
    ...
    <table id="tg_main"
        ...
        <thead>
        <tr>
            <th field="MaintainID" checkbox="true"></th>
            <th field="CustomerName" sortable="true" >客户</th>
            <th field="EmpName" sortable="true" >实施人</th>
            <th field="OperatorName" >跟进实施人</th>
            <th field="Content" sortable="true" >实施内容</th>
            <th field="TraceTime" sortable="true" >实施时间</th>
            <th field="ModifyTime" sortable="true" >跟进时间</th>
            <th field="Result"  >实施结果</th>
        </tr>
        </thead>
    </table>
    ...
    <script type="text/javascript">
        ...
        function btnEdit_Click(row_id) {
            //获取选中行的id
            if (!row_id) {
                row_id = get_checked_row("#tg_main", "MaintainID");
            }
            if (row_id) {
                top.showModal(
                        '/admin/Maintain/editMaintain?MaintainID=' + row_id,
                        '编辑', //调用语言来设置标题
                        {
                            width: 800,
                            height: 540, //窗口高度
        ...
        function btnAdd_Click() {
            top.showModal(
                    '/admin/Maintain/addMaintain'  , //使用C#生成url
                    '新增' , //调用语言来设置标题
                    {
                        width: 800,
                        height: 540, //窗口高度
        ...
        function btnRemove_Click() {
            //获取当前勾选的所有行id
            var row_ids = get_checked_rows("#tg_main", "MaintainID");
            if (row_ids && confirm_delete()) {
                $.post(
                        '/admin/Maintain/delMain' ,
                        {
                            ids : row_ids
                        },
                        function(data) {
