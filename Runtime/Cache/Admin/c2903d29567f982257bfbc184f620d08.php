        ...
        <ul class="nav" id="side-menu">
            ...
            <?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                    ...
                    <ul class="nav nav-second-level collapse">
                        <?php if(is_array($vo["son"])): $i = 0; $__LIST__ = $vo["son"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i; if(empty($s["son"])): ?><li>
                                    <a class="J_menuItem" href="<?php echo ($s["Url"]); ?>" data-index="0"><?php echo ($s["Title"]); ?></a>
                                </li>
                            <?php else: ?>
                                <li>
                                    <a href="">
                                        <i class="fa fa-user"></i>
                                        <span class="nav-label"><?php echo ($s["Title"]); ?></span>
                                        <span class="fa arrow"></span>
                                    </a>
                                    <ul class="nav nav-third-level collapse">
                                        <?php if(is_array($s["son"])): $i = 0; $__LIST__ = $s["son"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gs): $mod = ($i % 2 );++$i;?><li>
                                                <a class="J_menuItem" href="<?php echo ($gs["Url"]); ?>" data-index="6"><?php echo ($gs["Title"]); ?></a>
...
<div id="main" class="clearfix">
    ...
    <div class="w-admin">
        <div class="panel">
            ...
            <h2 class="text-success" style="padding-left: 15px;"><i class="fa fa-check-circle-o"></i> <?php echo($message); ?></h2>
            <?php }else{?>
            <h2 class="text-danger" style="padding-left: 15px;"><i class="fa fa-exclamation-triangle"></i> <?php echo($error); ?></h2>
            <?php }?>
            <?php if(empty($error_detail)): ?><div class="panel-body">
                    <blockquote>
                        <p>页面自动 <a id="href" href="<?php echo($jumpUrl); ?>">跳转</a> 等待时间： <b id="wait"><?php echo($waitSecond); ?></b></p>
                    </blockquote>
                </div>
                <?php else: ?>
                <div id="my_error"><?php echo ($error_detail); ?></div><?php endif; ?>
            <div class="panel-footer">
                <a href="<?php echo($jumpUrl); ?>"   id="" name="submitAddcustomer" class="btn btn-success">
                    <i class="fa fa-arrow-circle-o-right"></i> 立即跳转
                </a>
            </div>
        </div>
    </div>
